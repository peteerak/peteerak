/*!
 * Dashmix - v1.5.0
 * @author pixelcave - https://pixelcave.com
 * Copyright (c) 2018
 */
var globalData;
$(document).ready(setup);
function setup(){
    console.log("ready") 
    var option = {
        type: "Get",  
          url: "/Dashboard/mydata",  
          contentType: "application/json; charset=utf-8",  
          dataType: "json",  
          success: data_arrived 
    };
    $.ajax(option);
}

function data_arrived(data){
    console.log("arrived", data);
    globalData = data;
    console.log("globe", globalData)
    data_ready();
}

function data_ready(){ // start when ready
! function(r) {
    var e = {};

    function t(o) {
        if (e[o]) return e[o].exports;
        var n = e[o] = {
            i: o,
            l: !1,
            exports: {}
        };
        return r[o].call(n.exports, n, n.exports, t), n.l = !0, n.exports
    }
    t.m = r, t.c = e, t.d = function(r, e, o) {
        t.o(r, e) || Object.defineProperty(r, e, {
            enumerable: !0,
            get: o
        })
    }, t.r = function(r) {
        "undefined" != typeof Symbol && Symbol.toStringTag && Object.defineProperty(r, Symbol.toStringTag, {
            value: "Module"
        }), Object.defineProperty(r, "__esModule", {
            value: !0
        })
    }, t.t = function(r, e) {
        if (1 & e && (r = t(r)), 8 & e) return r;
        if (4 & e && "object" == typeof r && r && r.__esModule) return r;
        var o = Object.create(null);
        if (t.r(o), Object.defineProperty(o, "default", {
                enumerable: !0,
                value: r
            }), 2 & e && "string" != typeof r)
            for (var n in r) t.d(o, n, function(e) {
                return r[e]
            }.bind(null, n));
        return o
    }, t.n = function(r) {
        var e = r && r.__esModule ? function() {
            return r.default
        } : function() {
            return r
        };
        return t.d(e, "a", e), e
    }, t.o = function(r, e) {
        return Object.prototype.hasOwnProperty.call(r, e)
    }, t.p = "", t(t.s = 34)
}({
    34: function(r, e, t) {
        r.exports = t(35)
    },
    35: function(r, e) {
        function t(r, e) {
            for (var t = 0; t < e.length; t++) {
                var o = e[t];
                o.enumerable = o.enumerable || !1, o.configurable = !0, "value" in o && (o.writable = !0), Object.defineProperty(r, o.key, o)
            }
        }
        var o = function() {
            function r() {
                ! function(r, e) {
                    if (!(r instanceof e)) throw new TypeError("Cannot call a class as a function")
                }(this, r)
            }
            var e, o, n;
            return e = r, n = [{
                key: "initChartsBars",
                value: function() {
                    console.log(globalData, "globe in the scope");
                    var y_data = globalData[0];
                    var x_data = globalData[1];
                    Chart.defaults.global.defaultFontColor = "#495057", Chart.defaults.scale.gridLines.color = "transparent", Chart.defaults.scale.gridLines.zeroLineColor = "transparent", Chart.defaults.scale.ticks.beginAtZero = !0, Chart.defaults.global.elements.line.borderWidth = 1, Chart.defaults.global.legend.labels.boxWidth = 12;
                    var r, e = jQuery(".js-chartjs-analytics-bars");
                    r = {
                        labels: x_data,
                        datasets: [{
                            label: "This Weekz",
                            fill: !0,
                            backgroundColor: "rgba(103, 114, 229, .75)",
                            borderColor: "rgba(103, 114, 229, 1)",
                            pointBackgroundColor: "rgba(103, 114, 229, 1)",
                            pointBorderColor: "#fff",
                            pointHoverBackgroundColor: "#fff",
                            pointHoverBorderColor: "rgba(103, 114, 229, 1)",
                            data: y_data
                        }, ]
                    }, e.length && new Chart(e, {
                        type: "bar",
                        data: r
                    })
                }
            }, {
                key: "init",
                value: function() {
                    this.initChartsBars()
                }
            }], (o = null) && t(e.prototype, o), n && t(e, n), r
        }();
        jQuery(function() {
            o.init()
        })
    }
});
}